...
export class CommonService {
	...
	constructor(@Inject(DOCUMENT) private document: HTMLDocument,private http: HttpClient,private titleService : Title,private meta: Meta,public loaderService: LoaderService,) {
        this.auth_token = localStorage.getItem("auth_token");
        //console.log('this.auth_token' + this.auth_token);
        this.SITE_CONSTANT['API_END_POINT'] = environment.apiUrl+'/api/';
    ...
    public generateRequestHeadersAuthorise(urlEncoded : boolean = true,loadertruefalse : boolean = true){

        
        this.loaderService.setLoading(loadertruefalse);
        this.headers = new HttpHeaders( 
            {
    ...
    getList(url: string, options: any){
    	return this.http.get(this.SITE_CONSTANT['API_END_POINT']+url,options);
    }
    ...
    public getProfileOld(userId : any) : Promise<any>{
        let body = {
            userId:userId
        };
        let options = this.generateRequestHeadersAuthorise();
        return this.http
        .post(this.SITE_CONSTANT['API_END_POINT']+'getProfile',body,options)
        .toPromise()
        .then(this.extractData)
        .catch(this.handleError);
    } 

    public getAllEvent(userId : any) : Promise<any>{
        let body = {
            userId:userId
        };
        let options = this.generateRequestHeadersAuthorise();
        return this.http
        .post(this.SITE_CONSTANT['API_END_POINT']+'getAllAddedEvent',body,options)
        .toPromise()
        // .then((response: any )=>{
    ...
    public getChatData(chatId : any) : Promise<any>{
        let body = {
            chatId:chatId
        };
        let options = this.generateRequestHeadersAuthorise(true,false);
        return this.http
        .post(this.SITE_CONSTANT['API_END_POINT']+'getChatData',body,options)
        .toPromise()
        // .then((response: any )=>{
    ...
    public CheckIsEligibleForChat(userId : any,eventId : any) : Promise<any>{
        let body = {
            userId:userId,
            eventId:eventId
        };
        let options = this.generateRequestHeadersAuthorise();
        return this.http
        .post(this.SITE_CONSTANT['API_END_POINT']+'iseligibleChat',body,options)
        .toPromise()
    }

    public getProfile(userId : any) : Promise<any>{
        let body = {
            userId:userId
        };
        let options = this.generateRequestHeadersAuthorise();
    ...
    public getBanners() : Promise<any>{
        let options = this.generateRequestHeadersAuthorise(true,false);
        return this.http
        .get(this.SITE_CONSTANT['API_END_POINT']+'getBanners',options)
        .toPromise()
    }

    public getCmsPages() : Promise<any>{
        let options = this.generateRequestHeadersAuthorise(true,false);
        return this.http
        .get(this.SITE_CONSTANT['API_END_POINT']+'cmsPages',options)
        .toPromise()
    }

    public getCmsDetail(cmsSlug:any) : Promise<any>{
        let body = {
            cmsSlug:cmsSlug
        };
        let options = this.generateRequestHeadersAuthorise();
        return this.http
        .post(this.SITE_CONSTANT['API_END_POINT']+'cmsSingle',body,options)
        .toPromise()
    } 

    public viewProfile(userId : any) : Promise<any>{
        let body = {
            userId:userId
        };
        let options = this.generateRequestHeadersAuthorise();
        return this.http
        .post(this.SITE_CONSTANT['API_END_POINT']+'viewProfile',body,options)
        .toPromise()
    } 
    ...
    public setMetaTags(metaArray : any){
    	let description = this.checkVar(metaArray.description) != '' ? metaArray.description : 'The Selfwish';
    	let viewport = this.checkVar(metaArray.viewport) != '' ? metaArray.viewport : 'width=device-width, initial-scale=1';
    	let author = this.checkVar(metaArray.author) != '' ? metaArray.author : 'The Selfwish';
    	let keywords = this.checkVar(metaArray.keywords) != '' ? metaArray.keywords : 'The Selfwish';


        let ogtype = this.checkVar(metaArray.ogtype) != '' ? metaArray.ogtype : 'website';
        let ogtitle = this.checkVar(metaArray.ogtitle) != '' ? metaArray.ogtitle : 'The Selfwish';
        let ogdesc = this.checkVar(metaArray.ogdesc) != '' ? metaArray.ogdesc : 'The Selfwish';
        let ogimgwidth = this.checkVar(metaArray.ogimgwidth) != '' ? metaArray.ogimgwidth : 1200;
        let ogimgheight = this.checkVar(metaArray.ogimgheight) != '' ? metaArray.ogimgheight : 630;
        let ogimage = this.checkVar(metaArray.ogimage) != '' ? metaArray.ogimage : '';
        let ogurl = this.checkVar(metaArray.ogurl) != '' ? metaArray.ogurl : '';
        let ogsecure_url = this.checkVar(metaArray.ogsecure_url) != '' ? metaArray.ogsecure_url : '';

        let twitter_card = this.checkVar(metaArray.twitter_card) != '' ? metaArray.twitter_card : '';
        let twitter_title = this.checkVar(metaArray.twitter_title) != '' ? metaArray.twitter_title : '';
        let twitter_img = this.checkVar(metaArray.twitter_img) != '' ? metaArray.twitter_img : '';
        let twitter_desc = this.checkVar(metaArray.twitter_desc) != '' ? metaArray.twitter_desc : '';
    	...
    	this.meta.addTags([
            ...
            {name: 'og:image:width', content: ogimgwidth},
            {name: 'og:image:height', content: ogimgheight},
            {name: 'og:image', content: ogimage},
            {name: 'og:url', content: ogurl},
            {name: 'og:image:secure_url', content: ogsecure_url},

            {name: 'twitter:card', content: twitter_card},
            {name: 'twitter:title', content: twitter_title},
            {name: 'twitter:image', content: twitter_img},
