...
export class ActivityTypeListComponent {
    ...
    ngOnInit(): void {
      ...
      this.typeofactivity = this.activatedRoute.snapshot.params["typeofactivity"] ;
      if(this.typeofactivity == 'hot-activities'){
        this.page_title = 'Hot Activities';        
      }else{
        this.page_title = 'Upcoming Activities';
      }      
      this.commonService.setTitle(this.page_title);
      this.headerService.setTitle(this.page_title,'');
      this.getEventList();
    ...
    getEventList(page: number = 1) {
      ...
      let options = this.commonService.generateRequestHeadersAuthorise(false);
        ...
        this.commonService.postList('getEventTypeWise',body,options).subscribe((res: any) => {
          console.log(res);
