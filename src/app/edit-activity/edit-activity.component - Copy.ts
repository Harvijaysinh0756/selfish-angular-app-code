...
export class EditActivityComponent {
    ...
    public globalArticleDetail: any = [];
    categoryList: any = [];
    ...
    getCategory(page: number = 1) {
      ...
      let options = this.commonService.generateRequestHeadersAuthorise(false);
        ...
        this.commonService.getList('getCategories',options).subscribe((res: any) => {
          ...
          //jQuery('.selectpicker').selectpicker('val', 7);
          
          setTimeout(() => {
            //jQuery('.selectpicker option[value=7]').attr('selected','selected');
            //jQuery('.selectpicker').val(7).selectpicker("refresh");;
            jQuery('.selectpicker').val('7').selectpicker("refresh");            
            jQuery('.selectpicker').selectpicker('val','7');
    ...
    public setDefaultArticleValue(preserve: boolean = false) {
        ...
        console.log('this.globalArticleDetail.category_id'+this.EventDetail.category_id);
        console.log('this.globalArticleDetail.category_id'+this.commonService.checkVar(this.EventDetail.category_id));
        this.EventDetail.category_id = 
        this.commonService.checkVar(this.EventDetail.category_id) != '' ? this.commonService.checkVar(this.EventDetail.category_id) : '';
        
        if (preserve) {
            for (var k in this.EventDetail) {
                if (this.EventDetail.hasOwnProperty(k)) {
                    this.frmPostEvent.controls['category_id'].patchValue(7);
                }
            }
        }
    ...
    getEventDetail() {
      ...
      let options = this.commonService.generateRequestHeaders(false);
        ...
        this.frmPostEvent = this.formBuilder.group({
          'category_id' : ['9', [Validators.required]],
